...
def infer_fast(net, img, net_input_height_size, stride, upsample_ratio, cpu,
    ...
    print(str(img.shape)+'aaaaaaaaaaaaaaaaa')
...
def run_demo(net,action_net, image_provider, height_size, cpu, boxList):
    ...
    i = 0
    image_provider = np.expand_dims(image_provider,0)
    print(str(image_provider.shape)+" ccccccccccccccccccccccc")

    # fallFlag = 0
    # if i % 1 == 0:
    #         heatmaps, pafs, scale, pad = infer_fast(net, image_provider, height_size, stride, upsample_ratio, cpu)  # 返回热图,paf,输入模型图象相比原始图像缩放倍数,输入模型图像padding尺寸
    ...
    #             pose.img_pose = pose.draw(image_provider,is_save=True,show_draw=True)
    ...
    #                 cv2.rectangle(image_provider, (pose.bbox[0], pose.bbox[1]),
    #                               (pose.bbox[0] + pose.bbox[2], pose.bbox[1] + pose.bbox[3]), (0, 0, 255),thickness=3)
    #                 cv2.putText(image_provider, 'state: {}'.format(pose.pose_action), (pose.bbox[0], pose.bbox[1] - 16),
    ...
    #         if fallFlag == 1:
    #             t = time.time()
    #             cv2.imwrite(f'C:\\Users\\BUPT\\Desktop\\gao\\ism_person_openpose-master\\data\\testImg\\{t}.jpg', image_provider)
    #             print('我保存照片了')


    #         img = cv2.addWeighted(image_provider, 0.6, image_provider, 0.4, 0)
    #         # 保存识别后的照片
    #         cv2.imwrite(f'C:\\Users\\BUPT\\Desktop\\gao\\ism_person_openpose-master\\data\\testImg\\88888.jpg', image_provider)
    #         # print('我保存照片了')
    #         cv2.imshow('Lightweight Human Pose Estimation Python Demo', image_provider)
...
################################################
    ...
    for img in image_provider:   # 遍历图像集
        print(str(img.shape) + 'bbbbbbbbbbbbb')
        ...
        if i % 1 == 0:
            ...
            if fallFlag == 1:
                global t
                t= time.time()
                # cv2.imwrite(f'C:\\Users\\BUPT\\Desktop\\gao\\ism_person_openpose-master\\data\\finalImg\\{t}.jpg', img)
                # print('我保存照片了')
...
def detect_main(video_source = '',image_source = '',video_name=''):
    ...
    parser.add_argument('--images', nargs='+',
                        default='C:\\zqr\\project\\openpose_fall_detect_222\\data\\testImg',
...
def runPose(image,boxList,video_name=''):
    ...
    run_demo(net, action_net, frame_provider, args.height_size, args.cpu,boxList)
